...
class Settings(BaseSettings):
    ...
    return_on_capital_threshold: float = float(os.getenv("RETURN_ON_CAPITAL_THRESHOLD", "20.0"))
    capital_usdt: float = float(os.getenv("CAPITAL_USDT", "50000.0"))
    leverage: int = int(os.getenv("LEVERAGE", "20"))
    risk_free_rate: float = float(os.getenv("RISK_FREE_RATE", "0.05"))  # 5% годовых
    ...
    def update_from_dict(self, data: Dict) -> bool:
        ...
        try:
            ...
            if "risk_free_rate" in data:
                self.risk_free_rate = float(data["risk_free_rate"])
...
def update_config(config_dict: Dict):
    ...
    if "capital_usdt" in config_dict:
        try:
            capital = float(config_dict["capital_usdt"])
            if capital < 100 or capital > 10000000:
                return False, "Капитал должен быть от 100 до 10,000,000 USDT"
        except (ValueError, TypeError):
            return False, "Неверное значение для капитала"
    
    if "leverage" in config_dict:
        try:
            leverage = int(config_dict["leverage"])
            if leverage < 1 or leverage > 100:
                return False, "Плечо должно быть от 1 до 100"
        except (ValueError, TypeError):
            return False, "Неверное значение для плеча"
    
    if "risk_free_rate" in config_dict:
        try:
            rate = float(config_dict["risk_free_rate"])
            if rate < 0 or rate > 1:
                return False, "Безрисковая ставка должна быть от 0 до 1 (0% до 100%)"
        except (ValueError, TypeError):
            return False, "Неверное значение для безрисковой ставки"
